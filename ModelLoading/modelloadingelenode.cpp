...
void ModelLoadingEleNode::readHeaderEle(){
...
void ModelLoadingEleNode::readHeaderNode(){
	...
	for(int i=0;i< numberOfAttributesPerNode;i++) {
		std::shared_ptr<ScalarFieldDef> propertyFieldDef(
					new ScalarFieldDef(propertyIndex++,
									   "Property #%d"+std::to_string(i),
									   numberOfNodes));
		vertexProperties.push_back(propertyFieldDef);
	}
}
bool ModelLoadingEleNode::readVertices( PolygonMesh* mesh){
	...
	for(int i=0 ; i<numberOfAttributesPerNode; i++) {
		...
		vertexProperties[i]->expandBounds(prop);
	}
...
bool ModelLoadingEleNode::readPolygons( PolygonMesh* mesh){
	scanner.reset(fileSizeEle);
...
bool ModelLoadingEleNode::readPolyhedrons( PolyhedronMesh* mesh){
	scanner.reset(fileSizeEle);
	scanner.readInt(fileBufferEle,&numberOfElements);
	scanner.skipToNextLine(fileBufferEle);
	int polygonIndex = 0;
