...
Model* PropertyFieldLoadingPly::load(std::string filename, Model* model){
	fileBuffer = FileUtils::getFileToBuffer(filename,&fileSize);
	scanner.reset(fileSize);
...
bool PropertyFieldLoadingPly::readHeader(PolygonMesh* polygonMesh)
{
...
bool PropertyFieldLoadingPly::readPolygons( PolygonMesh* pol ) {
	int npf, pi;
...
bool PropertyFieldLoadingPly::readVertices( VertexCloud* pol ) {
...
vis::Vertex* PropertyFieldLoadingPly::readVertex(int id, float& x, float& y, float& z) {
	x = 0.0f;
	y = 0.0f;
	z = 0.0f;
...
bool PropertyFieldLoadingPly::readAdditionalEdges( VertexCloud* pol ) {
...
bool PropertyFieldLoadingPly::readPolygonsBinary( PolygonMesh* pol ) {
	unsigned char npf;
	unsigned int pi;
	std::vector<vis::Polygon*> &vecPol = pol->getPolygons();
	vecPol.reserve(pol->getPolygonsCount());
...
bool PropertyFieldLoadingPly::readVerticesBinary( VertexCloud* pol ) {
	std::vector<vis::Vertex*> &v = pol->getVertices();
	std::vector<float> &bounds = pol->getBounds();
	v.clear();
	v.reserve( pol->getVerticesCount() );
...
bool PropertyFieldLoadingPly::readBody( PolygonMesh* pol )
{
	if(isBinary){
